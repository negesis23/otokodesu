 ...
 home: function() {
  fetch("https://otakudesu-unofficial-api.rzkfyn.tech/api/v1/home").then((r)=>r.json()).then((j) => {
   ...
   j.data.ongoing_anime.forEach((i)=> {
    ...
    li.innerHTML = `<a style="border-color:hsla(var(--bc) / 0.2)" href="/anime/?slug=${i.slug}" class="flex flex-row gap-4 bg-base-100 rounded-box px-4 pl-0 overflow-hidden hover:bg-primary hover:text-primary-content items-center h-28 transition duration-150 ease-in-out border" alt="${i.title}">
    <img loading="lazy" src="https://images.weserv.nl/?url=${i.poster}&q=10&w=225&h=318&fit=contain&cbg=black&output=webp&default=https://i.ibb.co/CsgfGYT/noImage.jpg" alt="${i.title}" title="${i.title}" class="h-28 bg-base-200" height="112" width="79.24">
    <div class="flex-col flex gap-1 py-2">
   ...
   j.data.complete_anime.forEach((i) => {
    let li = document.createElement("li");
        li.style.animation = "fadeInUp .4s ease-in-out";
    li.innerHTML = `<a style="border-color:hsla(var(--bc) / 0.07)" href="/anime/?slug=${i.slug}" class="flex flex-row gap-4 bg-base-100 rounded-box px-4 pl-0 overflow-hidden hover:bg-primary hover:text-primary-content items-center h-36 transition duration-150 ease-in-out border-2" alt="${i.title}" title="${i.title}">
    <img loading="lazy" src="https://images.weserv.nl/?url=${i.poster}&q=10&w=225&h=318&fit=contain&cbg=black&output=webp&default=https://i.ibb.co/CsgfGYT/noImage.jpg" alt="${i.title}" title="${i.title}" class="h-36 bg-base-200" height="144" width="101.89">
 ...
 ongoing: function(p) {
  ...
  fetch(`https://otakudesu-unofficial-api.rzkfyn.tech/api/v1/ongoing-anime/${p ? p: ""}`).then((r)=>r.json()).then((j) => {
   ...
   j.data.forEach((i)=> {
    let li = document.createElement("li");
    li.style.animation = "fadeInUp .4s ease-in-out";
    li.innerHTML = `<a style="border-color:hsla(var(--bc) / 0.07)" href="/anime/?slug=${i.slug}" class="flex flex-row gap-4 bg-base-100 rounded-box px-4 pl-0 overflow-hidden hover:bg-primary hover:text-primary-content items-center h-36 transition duration-150 ease-in-out border-2" alt="${i.title}">
    <img loading="lazy" src="https://images.weserv.nl/?url=${i.poster}&q=10&w=225&h=318&fit=contain&cbg=black&output=webp&default=https://i.ibb.co/CsgfGYT/noImage.jpg" alt="${i.title}" title="${i.title}" class="h-36 bg-base-200" height="144" width="101.89">
    <div class="flex-col flex gap-1.5 py-2">
    <span class="flex flex-row gap-1 items-center "><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="w-5 h-5"><path fill-rule="evenodd" d="M2.25 12c0-5.385 4.365-9.75 9.75-9.75s9.75 4.365 9.75 9.75-4.365 9.75-9.75 9.75S2.25 17.385 2.25 12zm14.024-.983a1.125 1.125 0 010 1.966l-5.603 3.113A1.125 1.125 0 019 15.113V8.887c0-.857.921-1.4 1.671-.983l5.603 3.113z" clip-rule="evenodd" /></svg> ${i.current_episode}</span>
    <h3 class="font-medium text-lg line-clamp-2">${i.title}</h3>
 ...
 completed: function(p) {
  ...
  fetch(`https://otakudesu-unofficial-api.rzkfyn.tech/api/v1/complete-anime/${p?p: ""}`).then((r)=>r.json()).then((j) => {
   ...
   j.data.forEach((i)=> {
    ...
    <div class="flex-col flex gap-1.5 py-2">
    <span class="flex flex-row gap-1 items-center "><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="w-5 h-5"><path fill-rule="evenodd" d="M2.25 12c0-5.385 4.365-9.75 9.75-9.75s9.75 4.365 9.75 9.75-4.365 9.75-9.75 9.75S2.25 17.385 2.25 12zm14.024-.983a1.125 1.125 0 010 1.966l-5.603 3.113A1.125 1.125 0 019 15.113V8.887c0-.857.921-1.4 1.671-.983l5.603 3.113z" clip-rule="evenodd" /></svg> ${i.episode_count} Episode</span>
    <h3 class="font-medium text-lg line-clamp-2">&zwnj;${i.title}</h3>
    <span class=" flex flex-row gap-0.5 items-center"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 26 26" fill="currentColor" class="w-5 h-5"><path fill-rule="evenodd" d="M10.788 3.21c.448-1.077 1.976-1.077 2.424 0l2.082 5.007 5.404.433c1.164.093 1.636 1.545.749 2.305l-4.117 3.527 1.257 5.273c.271 1.136-.964 2.033-1.96 1.425L12 18.354 7.373 21.18c-.996.608-2.231-.29-1.96-1.425l1.257-5.273-4.117-3.527c-.887-.76-.415-2.212.749-2.305l5.404-.433 2.082-5.006z" clip-rule="evenodd" /></svg> ${i.rating} • ${i.last_release_date}</span>
    </div>
    </a>`;
    completedSection.querySelector("ul").appendChild(li);
  ...
  try {
   if (slug) {
    ...
    if (!document.querySelector("#loadMore")) {
     ...
     <div class="flex flex-row items-center bg-base-100 rounded-t-box p-3 border" style="border-color:hsla(var(--bc) / 0.2)">
     ...
     <a class="btn btn-sm btn-square btn-primary flex-none" href="/genres"><svg xmlns="http://www.w3.org/2000/svg" fill="currentColor" class="bi bi-tags-fill w-5 h-5" style="padding:1px" viewBox="0 0 16 16"><path d="M2 2a1 1 0 0 1 1-1h4.586a1 1 0 0 1 .707.293l7 7a1 1 0 0 1 0 1.414l-4.586 4.586a1 1 0 0 1-1.414 0l-7-7A1 1 0 0 1 2 6.586V2zm3.5 4a1.5 1.5 0 1 0 0-3 1.5 1.5 0 0 0 0 3z"/> <path d="M1.293 7.793A1 1 0 0 1 1 7.086V2a1 1 0 0 0-1 1v4.586a1 1 0 0 0 .293.707l7 7a1 1 0 0 0 1.414 0l.043-.043-7.457-7.457z"/> </svg></a>
     </div>
     <ul class="bg-base-100 p-3 flex flex-col gap-2 md:grid md:grid-cols-2 md:gap-3 border-x" style="border-color:hsla(var(--bc) / 0.2)">
     ...
     <div class="flex flex-row items-center bg-base-100 rounded-b-box p-3 border" style="border-color:hsla(var(--bc) / 0.2)">
    ...
    if (j.genreAnime.length) {
     ...
     j.genreAnime.forEach((i)=> {
      let li = document.createElement("li");
    li.style.animation = "fadeInUp .4s ease-in-out";
      li.innerHTML = `<a style="border-color:hsla(var(--bc) / 0.07)" href="/anime/?slug=${i.link}" class="flex flex-row gap-4 bg-base-100 rounded-box px-4 pl-0 overflow-hidden hover:bg-primary hover:text-primary-content items-center h-36 transition duration-150 ease-in-out border-2" alt="${i.title}" title="${i.title}"><img loading="lazy" src="https://images.weserv.nl/?url=${i.thumb}&q=10&w=225&h=318&fit=contain&cbg=black&output=webp&default=https://i.ibb.co/CsgfGYT/noImage.jpg" alt="${i.title}" title="${i.title}" class="h-36 bg-base-200" height="144" width="101.89"><div class="flex-col flex gap-1.5 py-2"><span class="flex flex-row gap-1 items-center "><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 25 25" fill="currentColor" class="w-5 h-5"><path fill-rule="evenodd" d="M2.25 12c0-5.385 4.365-9.75 9.75-9.75s9.75 4.365 9.75 9.75-4.365 9.75-9.75 9.75S2.25 17.385 2.25 12zm14.024-.983a1.125 1.125 0 010 1.966l-5.603 3.113A1.125 1.125 0 019 15.113V8.887c0-.857.921-1.4 1.671-.983l5.603 3.113z" clip-rule="evenodd" /></svg>${i.episode.match('Unknown') ? "Tidak Diketahui": i.episode.replace("Eps", "Episode")}</span><h3 class="font-medium text-lg line-clamp-2">${i.title}</h3><div class="flex flex-row gap-1 items-center line-clamp-1 break-words"><span class="flex flex-row items-center gap-0.5"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 26 26" fill="currentColor" class="w-5 h-5"><path fill-rule="evenodd" d="M10.788 3.21c.448-1.077 1.976-1.077 2.424 0l2.082 5.007 5.404.433c1.164.093 1.636 1.545.749 2.305l-4.117 3.527 1.257 5.273c.271 1.136-.964 2.033-1.96 1.425L12 18.354 7.373 21.18c-.996.608-2.231-.29-1.96-1.425l1.257-5.273-4.117-3.527c-.887-.76-.415-2.212.749-2.305l5.404-.433 2.082-5.006z" clip-rule="evenodd" /></svg> ${(i.rating?i.rating: "?")}</span> • <span class="line-clamp-1 text-ellipsis">${i.studio}</span></div></div></a>`;
    ...
    <div class="rounded-t-box p-3 bg-base-100 border flex flex-row items-center" style="border-color:hsla(var(--bc) / 0.2)">
  ...
  try {
   ...
   if (j.batch.title) {
    ...
    document.querySelector("main").innerHTML = `<section class="flex flex-col">
     ...
     <ul class="bg-base-100/70 menu p-3 gap-3 md:grid md:grid-cols-3 md:gap-3 border-x border-b rounded-b-box" style="border-color:hsla(var(--bc) / 0.2)">
      <div class="flex flex-col" style="animation: fadeInUp .3s ease-in-out;">
      <h3 class="font-medium border p-3 rounded-t-box bg-base-100" style="border-color:hsla(var(--bc) / 0.2)">${j.batch.download_list.low_quality.quality} (${j.batch.download_list.low_quality.size})</h3>
      <div class="border-x border-b rounded-b-box p-3 space-y-1.5 bg-base-100" style="border-color:hsla(var(--bc) / 0.2)">
      ${j.batch.download_list.low_quality.download_links.map((i) => '<li><a href="'+i.link+'" class="border" style="animation: fadeInUp .4s ease-in-out;border-color:hsla(var(--bc) / 0.2)"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="w-6 h-6" style="padding:1px"><path fill-rule="evenodd" d="M12 2.25a.75.75 0 01.75.75v11.69l3.22-3.22a.75.75 0 111.06 1.06l-4.5 4.5a.75.75 0 01-1.06 0l-4.5-4.5a.75.75 0 111.06-1.06l3.22 3.22V3a.75.75 0 01.75-.75zm-9 13.5a.75.75 0 01.75.75v2.25a1.5 1.5 0 001.5 1.5h13.5a1.5 1.5 0 001.5-1.5V16.5a.75.75 0 011.5 0v2.25a3 3 0 01-3 3H5.25a3 3 0 01-3-3V16.5a.75.75 0 01.75-.75z" clip-rule="evenodd" /></svg> '+i.host+'</a></li>').join("")}
      </div>
      </div>
      <div class="flex flex-col" style="animation: fadeInUp .3s ease-in-out;">
      <h3 class="font-medium border p-3 rounded-t-box bg-base-100" style="border-color:hsla(var(--bc) / 0.2)">${j.batch.download_list.medium_quality.quality} (${j.batch.download_list.medium_quality.size})</h3>
      <div class="border-x border-b rounded-b-box p-3 space-y-1.5 bg-base-100" style="border-color:hsla(var(--bc) / 0.2)">
      ${j.batch.download_list.medium_quality.download_links.map((i) => '<li><a href="'+i.link+'" class="border" style="animation: fadeInUp .4s ease-in-out;border-color:hsla(var(--bc) / 0.2)"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="w-6 h-6" style="padding:1px"><path fill-rule="evenodd" d="M12 2.25a.75.75 0 01.75.75v11.69l3.22-3.22a.75.75 0 111.06 1.06l-4.5 4.5a.75.75 0 01-1.06 0l-4.5-4.5a.75.75 0 111.06-1.06l3.22 3.22V3a.75.75 0 01.75-.75zm-9 13.5a.75.75 0 01.75.75v2.25a1.5 1.5 0 001.5 1.5h13.5a1.5 1.5 0 001.5-1.5V16.5a.75.75 0 011.5 0v2.25a3 3 0 01-3 3H5.25a3 3 0 01-3-3V16.5a.75.75 0 01.75-.75z" clip-rule="evenodd" /></svg> '+i.host+'</a></li>').join("")}
      </div>
      </div>
      <div class="flex flex-col" style="animation: fadeInUp .3s ease-in-out;">
      <h3 class="font-medium border p-3 rounded-t-box bg-base-100" style="border-color:hsla(var(--bc) / 0.2)">${j.batch.download_list.high_quality.quality} (${j.batch.download_list.high_quality.size})</h3>
      <div class="border-x border-b rounded-b-box p-3 space-y-1.5 bg-base-100" style="border-color:hsla(var(--bc) / 0.2)">
      ${j.batch.download_list.high_quality.download_links.map((i) => '<li><a href="'+i.link+'" class="border" style="animation: fadeInUp .4s ease-in-out;border-color:hsla(var(--bc) / 0.2)"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="w-6 h-6" style="padding:1px"><path fill-rule="evenodd" d="M12 2.25a.75.75 0 01.75.75v11.69l3.22-3.22a.75.75 0 111.06 1.06l-4.5 4.5a.75.75 0 01-1.06 0l-4.5-4.5a.75.75 0 111.06-1.06l3.22 3.22V3a.75.75 0 01.75-.75zm-9 13.5a.75.75 0 01.75.75v2.25a1.5 1.5 0 001.5 1.5h13.5a1.5 1.5 0 001.5-1.5V16.5a.75.75 0 011.5 0v2.25a3 3 0 01-3 3H5.25a3 3 0 01-3-3V16.5a.75.75 0 01.75-.75z" clip-rule="evenodd" /></svg> '+i.host+'</a></li>').join("")}
      </div>
      </div>
     </ul>
    </section>`;
 ...
 bookmarks: function(slug) {
  ...
  if (i.length > 0) {
   ...
   i.reverse().forEach((i) => {
    ...
    li.style.animation = "fadeInUp .4s ease-in-out";
    li.innerHTML = `<a style="border-color:hsla(var(--bc) / 0.07)" href="/anime/?slug=${i.slug}" class="flex flex-row gap-4 bg-base-100 rounded-l-box px-4 pl-0 overflow-hidden hover:bg-primary hover:text-primary-content items-center h-24 transition duration-150 ease-in-out border-y-2 border-l-2 w-full" alt="${i.title}">
    <img loading="lazy" src="https://images.weserv.nl/?url=${i.poster}&q=10&w=225&h=318&fit=contain&cbg=black&output=webp&default=https://i.ibb.co/CsgfGYT/noImage.jpg" alt="${i.title}" title="${i.title}" class="h-24 bg-base-200" height="96" width="67.92">
    <div class="flex-col flex gap-1.5 py-2">
    <h3 class="font-medium text-lg line-clamp-1">&zwnj;${i.title}</h3>
    <span class="line-clamp-1 text-ellipsis">${i.genres}</span>
    </div>
    </a>
    <button class="rounded-r-box p-1.5 bg-base-100 text-error hover:bg-error hover:text-error-content h-24 flex border-2 border-error/10 justify-center items-center" onclick="if(confirm('Hapus Anime Ini Dari Bookmark?')){bookmarks.remove('${i.slug}');alert('Anime Berhasil Dihapus Dari Bookmark.');this.parentNode.remove(); if(!document.querySelector('#bookmarks_lists li')){location.reload()}}">
 ...
 if (this.value < 1) {
  ...
  if (j.data.length > 0) {
   j.data.forEach((i)=> {
    ...
    resItem.innerHTML = `<a class="flex flex-row items-center gap-2.5 w-full rounded-box border-2 overflow-hidden hover:bg-primary hover:text-primary-content transition duration-150 ease-in-out" style="border-color:hsla(var(--bc) / 0.07)" href="/anime/?slug=${i.slug}" alt="${i.title}" title="${i.title}">
    <img loading="lazy" class="h-20 bg-base-200" width="53.76" height="76" src="https://images.weserv.nl/?url=${i.poster}&q=10&w=225&h=318&fit=contain&cbg=black&output=webp&default=https://i.ibb.co/CsgfGYT/noImage.jpg" alt="${i.title}" title="${i.title}">
    <div class="flex flex-col">
    <small class="flex flex-row items-center text-sm">${i.status.match("Ongoing") ? '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 26 26" fill="currentColor" class="w-5 h-5" style="padding:1px"><path fill-rule="evenodd" d="M12.963 2.286a.75.75 0 00-1.071-.136 9.742 9.742 0 00-3.539 6.177A7.547 7.547 0 016.648 6.61a.75.75 0 00-1.152-.082A9 9 0 1015.68 4.534a7.46 7.46 0 01-2.717-2.248zM15.75 14.25a3.75 3.75 0 11-7.313-1.172c.628.465 1.35.81 2.133 1a5.99 5.99 0 011.925-3.545 3.75 3.75 0 013.255 3.717z" clip-rule="evenodd" /></svg>': '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 26 26" fill="currentColor" class="w-5 h-5" style="padding:1px"><path fill-rule="evenodd" d="M8.603 3.799A4.49 4.49 0 0112 2.25c1.357 0 2.573.6 3.397 1.549a4.49 4.49 0 013.498 1.307 4.491 4.491 0 011.307 3.497A4.49 4.49 0 0121.75 12a4.49 4.49 0 01-1.549 3.397 4.491 4.491 0 01-1.307 3.497 4.491 4.491 0 01-3.497 1.307A4.49 4.49 0 0112 21.75a4.49 4.49 0 01-3.397-1.549 4.49 4.49 0 01-3.498-1.306 4.491 4.491 0 01-1.307-3.498A4.49 4.49 0 012.25 12c0-1.357.6-2.573 1.549-3.397a4.49 4.49 0 011.307-3.497 4.49 4.49 0 013.497-1.307zm7.007 6.387a.75.75 0 10-1.22-.872l-3.236 4.53L9.53 12.22a.75.75 0 00-1.06 1.06l2.25 2.25a.75.75 0 001.14-.094l3.75-5.25z" clip-rule="evenodd" /></svg>'} ${i.status}</small>
    <b class="line-clamp-1 font-medium">${i.title}</b>
    <span class="text-sm flex flex-row items-center gap-1 line-clamp-1 break-words"><small class="flex flex-row items-center text-sm"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 27 27" fill="currentColor" class="w-5 h-5" style="padding:1px"><path fill-rule="evenodd" d="M10.788 3.21c.448-1.077 1.976-1.077 2.424 0l2.082 5.007 5.404.433c1.164.093 1.636 1.545.749 2.305l-4.117 3.527 1.257 5.273c.271 1.136-.964 2.033-1.96 1.425L12 18.354 7.373 21.18c-.996.608-2.231-.29-1.96-1.425l1.257-5.273-4.117-3.527c-.887-.76-.415-2.212.749-2.305l5.404-.433 2.082-5.006z" clip-rule="evenodd" /> </svg>  ${i.rating?i.rating: "?"}</small> • <small class="line-clamp-1 text-sm">${i.genres.map((g)=>g.name).join(", ")}</small></span>
    </div>
    </a>`;
